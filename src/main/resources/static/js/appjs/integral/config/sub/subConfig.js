	...
	function configLoad(){
		var grid = $("#subConfigTab").bootstrapTable({
			method:"get",
			url:prefix + "/list",
			iconSize : 'outline',
...
//				
	...
	function reloadIntegralConfig(){
		$("#subConfigTab").bootstrapTable('refresh');
	}
	...
	function edit(id){
		layer.open({
			...
			content : '/integral/config/edit/2/' + id // iframe的url
		});
	}
	...
	function add(){
		layer.open({
			...
			shadeClose : false, // 点击遮罩关闭层
			area : [ '500px', '320px' ],//宽，高
			content : '/integral/config/add/2'// iframe的url
		});
	}
	...
	function batchRemove(){
		//获取选择的行数据
		let params = $('#subConfigTab').bootstrapTable('getSelections'); // 返回所有选择的行，当没有选择的记录时，返回一个空数组
