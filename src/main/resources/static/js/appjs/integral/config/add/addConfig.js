	...
	function configLoad(){
		var grid = $("#addConfigTab").bootstrapTable({
			method:"get",
			url:prefix + "/list",
			iconSize : 'outline',
			...
			columns:[
				{checkbox: true},
				{field: "integralTypeName", title:"添加积分类型"},
				{field: "integralCoefficient", title:"积分系数"},
...
//				
	...
	function reloadIntegralConfig(){
		$("#addConfigTab").bootstrapTable('refresh');
	}
	...
	function batchRemove(){
		//获取选择的行数据
		let params = $('#addConfigTab').bootstrapTable('getSelections'); // 返回所有选择的行，当没有选择的记录时，返回一个空数组
