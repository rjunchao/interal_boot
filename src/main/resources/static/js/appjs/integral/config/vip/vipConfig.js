	...
	function configLoad(){
		var grid = $("#vipConfigTab").bootstrapTable({
			method:"get",
			url:prefix + "/list",
			iconSize : 'outline',
			...
			columns:[
				{checkbox: true},
				{field: "integralTypeName", title:"兑换类型"},
				{field: "integralCoefficient", title:"兑换积分"},
				{field: "def2", title:"可赠次数"},
...
//				
	...
	function reloadIntegralConfig(){
		$("#vipConfigTab").bootstrapTable('refresh');
	}
	...
	function edit(id){
		layer.open({
			...
			area : [ '500px', '360px' ],//宽，高
			content : '/integral/config/edit/3/' + id // iframe的url
		});
	}
	...
	function add(){
		layer.open({
			...
			shadeClose : false, // 点击遮罩关闭层
			area : [ '500px', '360px' ],//宽，高
			content : '/integral/config/add/3'// iframe的url
		});
	}
	...
	function batchRemove(){
		//获取选择的行数据
		let params = $('#vipConfigTab').bootstrapTable('getSelections'); // 返回所有选择的行，当没有选择的记录时，返回一个空数组
