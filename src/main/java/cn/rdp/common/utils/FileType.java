...
public class FileType {
	public static int fileTypeNew(String fileName) {
		if (fileName == null) {
			fileName = "文件名为空！";
			return 500;
			...
			if(img.contains(fileType)) {return 0;}

			// 创建文档类型数组1
			String document = "txt, doc, docx, xls, htm, html, jsp, rtf, wpd, pdf, ppt" ;
			if(document.contains(fileType)) {return 1;}
			
			// 创建视频类型数组2
			String video = "mp4, avi, mov, wmv, asf, navi, 3gp, mkv, f4v, rmvb, webm" ;
			if(video.contains(fileType)) {return 2;}
			
			// 创建音乐类型数组3
			String music = "mp3, wma, wav, mod, ra, cd, md, asf, aac, vqf, ape, mid, ogg m4a, vqf";
			if(music.contains(fileType)) {return 3;}
